...
from core.news import generate_stock_analysis_stream, get_all_headlines, generate_ai_digest
...
from utils import (
    ...
    load_data, search_tickers, load_watchlist,
...
.nc{background:#fff;border:1px solid #eee;border-radius:8px;padding:12px;margin-bottom:6px}
.nc a{text-decoration:none;color:#111;font-weight:500}
.ai-report{background:#f9f9ff;border:1px solid #d0d8ff;border-radius:10px;
           padding:20px;font-size:0.9em;line-height:1.8;white-space:pre-wrap;
...
# サイドバー: 銘柄検索
# ============================================================
# Note: ウォッチリストはapp.pyのサイドバーで管理
with st.sidebar:
    st.markdown("---")
    st.markdown("**🔍 銘柄検索**")
    search_q = st.text_input("", placeholder="Apple, AAPL, トヨタ...",
                              key="an_sq", label_visibility="collapsed")
    if search_q and len(search_q) >= 2:
        with st.spinner("検索中..."):
            suggestions = search_tickers(search_q)
        if suggestions:
            labels = [""] + [f"{r['symbol']}  {r['name']}" for r in suggestions]
            sel = st.selectbox("", labels, key="an_sel", label_visibility="collapsed")
            if sel:
                sel_t = sel.split("  ")[0].strip()
                if st.button(f"「{sel_t}」を分析", type="primary",
                             use_container_width=True, key="an_go"):
                    st.session_state.active_ticker = sel_t
                    load_data.clear()
                    st.rerun()
        else:
            st.caption("候補なし")
    ...
    for i, (label, tkr) in enumerate(EXAMPLES.items()):
        if cols[i % 2].button(label, key=f"an_q_{tkr}", use_container_width=True):
            st.session_state.active_ticker = tkr
            load_data.clear()
            st.rerun()

    if "active_ticker" in st.session_state:
        st.info(f"📊 **{st.session_state.active_ticker}**")

# ============================================================
# メインコンテンツ
# ============================================================
st.markdown("# 📊 銘柄分析 & バリュエーション")

if "active_ticker" not in st.session_state:
    st.markdown("""
左サイドバーのウォッチリストをクリック、または上の検索ボックスから銘柄を選択してください。
...
with c1:
    st.markdown(f"## {cname}")
    st.caption(f"{ticker}　|　{sector} / {data.get('industry', '')}")
with c2:
...
# ============================================================
st.markdown("---")
ai_col, info_col = st.columns([2, 5])
with ai_col:
    ai_btn = st.button(
        "🤖 AI 総合分析レポートを生成",
        type="primary",
        use_container_width=True,
        help="業界・企業・競合・将来シナリオ・Tips を Claude Sonnet が分析します（30秒程度）",
    )
with info_col:
    st.info("業界分析 / 企業分析 / 競合比較 / 将来シナリオ3つ / 投資家Tips / 判断サマリー をそのままコピペできる形式で生成します")

if ai_btn or st.session_state.get(f"ai_report_{ticker}"):
    st.markdown("### 🤖 AI 投資分析レポート")

    if ai_btn:
        # ストリーミングで新規生成
        st.session_state.pop(f"ai_report_{ticker}", None)
        news_raw  = data.get("news") or []
        news_items = parse_news(news_raw)
        st.info("🔍 セクションが順次表示されます。完了まで約30〜60秒かかります。")
        report = st.write_stream(generate_stock_analysis_stream(data, news_items))
        st.session_state[f"ai_report_{ticker}"] = report
        st.rerun()  # 完了後にコピーボタン等を再描画

    elif st.session_state.get(f"ai_report_{ticker}"):
        # キャッシュ済みレポートを表示
        report = st.session_state[f"ai_report_{ticker}"]
        st.markdown(f'<div class="ai-report">{report}</div>', unsafe_allow_html=True)

        # コピー用テキストエリア
        with st.expander("📋 テキストをそのままコピー"):
            st.text_area("", value=report, height=500, key=f"copy_{ticker}")

        if st.button("🔄 レポートを再生成", key=f"regen_{ticker}"):
            st.session_state.pop(f"ai_report_{ticker}", None)
            st.rerun()
...
# ============================================================
st.markdown("### 📈 チャート")
st.caption("「インジケーター」ボタンで RSI / MACD / ボリンジャーバンドを追加できます")
...
with tab_dcf:
    ...
    if bench:
        st.info(f"**{sector} 参考値** — WACC: {bench['wacc']} ／ 成長率: {bench['growth']}")
    with st.expander("📊 企業参考データ", expanded=True):
        rc1, rc2, rc3, rc4 = st.columns(4)
        rc1.metric("推定WACC", f"{disc_def*100:.1f}%")
...
with tab_per:
    ...
    if bench: st.info(f"**{sector} 参考PER**: {bench['per']}倍")
    l, r = st.columns(2)
    with l:
        if per_cur: st.info(f"現在PER: {per_cur:.1f}x")
        per_target = st.slider("目標PER", 5.0, 80.0, float(min(max(per_cur or 15.0, 5.0), 80.0)), 0.5, key="pt", format="%.1fx")
        eps_in = st.number_input(f"EPS [{currency}]", value=float(eps) if eps > 0 else (1.0 if currency != "JPY" else 100.0), step=0.1 if currency != "JPY" else 1.0, key="pe")
    with r:
...
with tab_pbr:
    ...
    if bench: st.info(f"**{sector} 参考PBR**: {bench['pbr']}倍")
    l, r = st.columns(2)
    with l:
        if pbr_cur: st.info(f"現在PBR: {pbr_cur:.2f}x")
        pbr_target = st.slider("目標PBR", 0.5, 15.0, float(min(max(pbr_cur or 1.5, 0.5), 15.0)), 0.1, key="pbrt", format="%.2fx")
        bvps_in = st.number_input(f"1株純資産（BPS）[{currency}]", value=float(bvps) if bvps > 0 else (10.0 if currency != "JPY" else 1000.0), step=0.1 if currency != "JPY" else 1.0, key="bvpsi")
    with r:
...
with tab_graham:
    ...
    with l:
        ...
        g_eps  = st.number_input(f"EPS [{currency}]", value=float(eps) if eps > 0 else (1.0 if currency != "JPY" else 100.0), step=0.1 if currency != "JPY" else 1.0, key="geps")
    with r:
...
with tab_ddm:
    ...
    if dps <= 0: st.warning("⚠️ 現在無配または配当データなし")
    l, r = st.columns(2)
    with l:
        ddm_dps = st.number_input(f"1株配当（DPS）[{currency}]", value=float(dps) if dps > 0 else (1.0 if currency != "JPY" else 50.0), step=0.01 if currency != "JPY" else 1.0, key="ddps")
...
with tab_ev:
    ...
    l, r = st.columns(2)
    with l:
        ev_mult = st.slider("目標EV/EBITDA倍率", 4.0, 30.0, float(min(max(ev_cur or 12.0, 4.0), 30.0)), 0.5, key="em", format="%.1fx")
        ev_ebi  = st.number_input("EBITDA", value=float(ebitda) if ebitda > 0 else 1e9, format="%.3e", key="eeb")
        ev_nd   = st.number_input("純負債", value=float(net_debt), format="%.3e", key="end_")
        ev_sh   = st.number_input("発行済み株式数", value=float(shares) if shares > 0 else 1e9, format="%.3e", key="esh")
    with r:
...
if use_fcf > 0:
    ...
    scenarios = {
        "強気 🚀": {"g": min(dcf_growth+0.08, 0.30), "d": max(dcf_disc-0.02, 0.05)},
        "中立 ➡️":  {"g": dcf_growth, "d": dcf_disc},
        "弱気 🐻": {"g": max(dcf_growth-0.05, 0.01), "d": min(dcf_disc+0.03, 0.18)},
    }
    cols3 = st.columns(3)
    for col, (name, p) in zip(cols3, scenarios.items()):
        res = calc_dcf(cp, use_fcf, p["g"], dcf_term, p["d"], dcf_years)
        with col:
            if res.is_valid:
                ...
                <div style="background:#f8f9fa;border-radius:10px;padding:20px;text-align:center">
                    ...
                    <div style="color:#888;font-size:0.82em;margin-top:6px">成長率{p['g']*100:.0f}% / 割引率{p['d']*100:.0f}%</div>
                </div>
                """, unsafe_allow_html=True)
...
st.markdown("---")
st.markdown("### 📰 関連ニュース")
yt_url = f"https://www.youtube.com/results?search_query={url_quote(cname + ' 株価 分析')}"
gn_url = f"https://news.google.com/search?q={url_quote(cname + ' ' + ticker)}&hl=ja"
c1, c2 = st.columns(2)
with c1:
...
for item in news_items[:8]:
    if item.get("url") and item.get("title"):
        ...
        <div class="nc">
            ...
            <div style="color:#aaa;font-size:0.78em">{item.get('publisher','')}</div>
        </div>
        """, unsafe_allow_html=True)

st.markdown("---")
st.caption("⚠️ 投資教育目的のみ。データは yFinance / Claude (Anthropic) より取得。")
